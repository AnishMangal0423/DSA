...
public class q{
    public int diff(List<Integer> arr) {
        int ans = 2;
        int length = arr.size();
        if (length <= 2)
            return length;
        
        List<Integer> l = new ArrayList<>(Collections.nCopies(length, 2));
        ...
        for (int j = length - 2; j >= 0; j--) {
            ...
            while (i >= 0 && k < length) {
                if (arr.get(i) + arr.get(k) == 2 * arr.get(j)) {
                    l.set(j, Math.max(l.get(k) + 1, l.get(j)));
                    ans = Math.max(ans, l.get(j));
                    i -= 1;
                    k += 1;
